...
- $reviews\_per\_month$: El número de reviews de la base de datos en relación al tiempo que abarca.
...
## 04 Conversión de tipos incorrectos
...
data %>% count(host_is_superhost) # Como la variable es string, comprobaremos cuántos valores hay de cada uno, convertiremos, y los comprobaremos tras la conversión
data[data$host_is_superhost=="","host_is_superhost"] <- NA
data$host_is_superhost <- to_logical(data$host_is_superhost)
data %>% count(host_is_superhost)
...
ggplot(data, aes(x=price, y=..density..)) + geom_histogram(binwidth=10, fill="#69b3a2", color="#e9ecef", alpha=0.9) + geom_density(alpha = .3, fill="orange") + xlim(c(0,500))

ggplot(data, aes(y=price)) + geom_boxplot(varwidth = TRUE, alpha=0.2, col="orange") + scale_y_continuous(trans='log10')
...
## 07. Análisis detallado multivariante del precio


### 07.1 Subdivisión en categorías


#### 07.1.1 Ajustar al número de camas:
