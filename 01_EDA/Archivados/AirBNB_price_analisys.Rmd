...
Vemos que los tipos de datos de variables, como *host_since*, *last_review*, *host_is_superhost*, *price*, no coinciden con su contenido. Tambien en las variables (*host_since*, *host_is_superhost*, *description*, *last_review*)  hay valores perdidos, que los denotamos como NA.
...
```
   ...
   - formato *character* para *description*, valores perdidos los denotamos como NA 
```{r transform NA}
...
También nos interesará el número de alojamientos que tiene cada propietario.
Сalcularemos y anadimos la variable *host_listings_count*.

```{r transform listings count}
data <- data %>%
  add_count(host_id)
colnames(data)[colnames(data) == 'n'] <- 'host_listings_count'
```

Además del precio total del alquiler del alojamiento, estaremos interesados en el precio del alquiler por persona *price_per_person*.
```{r transform price/person}
data$price_per_person <- data$price/data$accommodates
```

El número de ID únicos de los objetos de ubicación es igual al número de observaciones, por lo que no tiene sentido seguir utilizando esta variable para el análisis. Pero esta variable sirve para calcular numero de listing que tiene un host. 
```{r id null}
data$id <- NULL
```
...
#write_csv(data_train_clean, file = "data_train_clean.csv")
