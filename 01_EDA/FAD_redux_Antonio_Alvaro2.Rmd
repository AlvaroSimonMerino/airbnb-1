...
kable(as.matrix(csna)) %>% kable_styling("striped")  %>% kable_styling("striped") %>% scroll_box(width= "100%", height = "350px")
```

<br/>

Para seleccionar un método de sustitución de valores faltantes, primero se analizará qué variables están altamente correlacionadas para excluírlas del proceso, puesto que pueden conducir a una multicolinealidad y, en consecuencia, dificultar su modelado:
...
Intentamos imputar datos faltantes de la variable *bedrooms* usando regresión lineal, ya que sabemos que hay correlacion alta entre esta variable y la variable *accommodates*.

```{r datos na line, echo=FALSE, message=FALSE, warning=FALSE}
fit <- with(data_train_del_na, lm(bedrooms ~ accommodates))


kable(as.matrix(summary(fit))) %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(width = "100%", height = "350px")
```
...
```{r datos na line result, echo=FALSE, message=FALSE, warning=FALSE}
...
kable(as.matrix(regr.eval(actuals, pred_lm))) %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(width = "100%")
```

El error porcentual absoluto medio (mape) fue 26,4%, que es un resultado insatisfactorio.
...
Como tenemos alta probabilidad de que una columna sea una combinación lineal de otra, vamos usar methodos *rf* y *cart* del paquete *mice*.
Los métodos de imputación predeterminados de *mice*, que implican regresión lineal, en nuestro caso van dar como resultado una matriz X que no se podrá invertir.

Utilizando este paquete, podemos imputar datos faltantes para todas variables que contienen NAs.
...
```{r datos imp rf na, echo=FALSE, message=FALSE, warning=FALSE}
anyNA(miceOutput) # False
```

Se confirma la ausencia de datos faltantes.
Vamos a comparar las distribuciones de los datos imputeados con los originales, usando el gráfico *densityplot*..
...
kable(as.matrix(miceOutput_cart)) %>%
  kable_styling("striped") %>%
  scroll_box(width = "100%", height = "350px")
```
...
Comprobamos si queda algún NA en el conjunto de datos.

```{r datos imp cart na, echo=FALSE, message=FALSE, warning=FALSE}
anyNA(miceOutput_cart) # False
```

Queda confirmada la ausencia de datos faltantes.
Vamos a comparar las distribuciones de los datos imputeados con los originales, usando el gráfico *densityplot*.
...
Podemos apreciar que los datos imputados son muy cercanos a lo datos observados, pero el resultado que más se ajusta es por el método *cart*.
Usamos este método para imputar los valores faltantes (NA) en nuestras variables.
...
Vamos a comprobar si tras las imputaciones, hemos conseguido eleminar todos los NAs de nuestra base de datos *data_train*.

```{r datos cart na, echo=FALSE, message=FALSE, warning=FALSE}
na_dat <- colSums(is.na(data_train_del_na)) # False
kable(as.matrix(na_dat)) %>%
  kable_styling("striped") %>%
  scroll_box(width = "100%", height = TRUE)
```

Observamos que *data_train* ya no tiene datos faltantes y podemos trabajar con los datos de una forma más eficaz.
...
#### Análisis del precio en función al distrito

```{r priceVSngc, fig.align="center", message=FALSE, warning=FALSE}
data_train %>%
  ggplot(aes(x=(reorder(neighbourhood_group_cleansed,price ,na.mr=TRUE)), y=price, fill=neighbourhood_group_cleansed)) + geom_boxplot() + scale_y_continuous(trans='log10') + coord_flip() + theme(legend.position="none") + xlab("Distritos") + ylab("Precio")


```

En el gráfico se aprecia que en la mayoría de casos, el precio se encuentra entre 50 y 100 dólares, y que hay una importante fluctuación, que a priori no parece tener mucho que ver con la localización especifica.
El caso que más llama la atención, es el del distrito de "San Blas - Canillejas" sin haber un motivo aparente por el que el precio fluctua tanto.
...
Y finalmente imprimimos sobre el mapa de madrid con un gradiente de color en función al precio
...
Vamos a realizar una transformación sobre la variable precio, para convertirla en precio por persona (y por noche).
...
Claramente, el **precio por persona fluctua menos** que el precio a la hora de compararlo **por distritos**.
En general, se aprecia como los precios se agrupan **entre 10 y 50 doláres** de media por persona y noche, con algún **caso extraño** como es el de ***"San Blas - Canillejas".***

<br/>

Vamos pues a profundizar en el porqué de esta diferencia tan significativa

```{r canillejas, fig.align="center", fig.height=15, fig.width=6, message=FALSE, warning=FALSE}

dataCan <- data_train[data_train$neighbourhood_group_cleansed=="San Blas - Canillejas",]
count(dataCan)

can <- ggplot(dataCan, aes(x=room_type, y=price_per_person, fill=room_type)) + geom_boxplot() + scale_y_continuous(trans='log10') + theme(legend.position="NONE") + ggtitle("Canillejas") + xlab("")
...
cerca_wanda <- ggplot(cercaWanda, aes(x=room_type, y=price_per_person, fill=room_type)) + geom_boxplot() + scale_y_continuous(trans='log10') + theme(legend.position="NONE") + ggtitle("Cerca de Wanda") + xlab("") # Resueltísimo: Habitaciones privadas y compartidas
lejos_wanda <- ggplot(lejosWanda, aes(x=room_type, y=price_per_person, fill=room_type)) + geom_boxplot() + scale_y_continuous(trans='log10')  + theme(legend.position="NONE") + ggtitle("Lejos de Wanda") + xlab("") # Resueltísimo

grid.arrange(can, cerca_wanda, lejos_wanda, nrow=3, ncol=1)
```

A partir de lo que observamos en los gráficos, buscamos la descripción de algunos de los airbnbs de Canillejas y descubrimos que mencionan en muchos de ellos que esta cerca del Wanda Metropolitano.
Realizamos dos gráficos más teniendo en cuenta la proximidad a Wanda, y claramente se puede concluir que el caso de Canillejas es que los airbnbs cercanos a Wanda fluctuan mucho más y nos descuadran el precio, además de tener claramente más opciones de alquiler de habitación que de apartamento entero.
...
ggplot(neighNoWanda, aes(x=neighbourhood_group_cleansed, y=price_per_person, fill=neighbourhood_group_cleansed)) + geom_boxplot() + scale_y_continuous(trans='log10') + coord_flip() + theme(legend.position = "NONE")
