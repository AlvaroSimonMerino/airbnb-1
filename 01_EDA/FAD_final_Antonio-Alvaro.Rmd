...
```{r mapa, eval=FALSE, fig.align="center", fig.height=7, fig.width=6.5, message=FALSE, warning=FALSE}
...
```{r precioGeneral, eval=FALSE, fig.align="center", message=FALSE, warning=FALSE}
...
cerca_wanda <- ggplot(cercaWanda, aes(x=room_type, y=price_per_person, fill=room_type)) + geom_boxplot() + scale_y_continuous(trans='log10') + theme(legend.position="NONE") + ggtitle("Cerca del estadio") + xlab("") + scale_fill_brewer(palette="RdGy")
...
# price vs host_since
price_hs <- ggplot(data_train, aes(x=host_exp_days, y=price)) + geom_col(binwidth = 10, fill='#8B1C62') + scale_y_continuous(trans='log', labels = comma) # CHECK: Irrelevant TODO: agregar por promedio 
...
En esta misma gráfica, podemos observar que se podría obtener un modelo con 16 predictores aproximadamente, cuyo error sería muy similar al del mejor modelo (que utiliza 39).
...
Se obtiene un SE de 0.001511 con 16 variables, y 0.001375 con 39.
...
Con Ridge, se obtiene un SE de 0.001405 con 44 variables.
...
Para *test*, el valor de R² fue 0.6221.
...
Сomo resultado, obtuvimos un MAPE del 8,8%, que es que ligeramente peor que para el *train* (8,6%).
