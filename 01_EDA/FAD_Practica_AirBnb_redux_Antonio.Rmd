...
Para la realización de esta práctica se ha seleccionado el dataset *listings.csv*  (Airbnb Madrid, 2021-09-10), una base de datos obtenida de *insideairbnb.com* con los alquileres de AirBnb de Madrid. Esta base de datos es de dominio público y consta de 74 variables con 18909 observaciones. Puede ser descargada a través del siguiente enlace: http://data.insideairbnb.com/spain/comunidad-de-madrid/madrid/2021-09-10/data/listings.csv.gz

<br/>

Las librerías usadas para esta práctica son las siguientes:
...
## 01 Introducción a la práctica TODO: resumir y/o graficar
...
### Objetivos generales
- Analizar las variables del conjunto de datos.
- Aplicar las transformaciones necesarias cara a utilizar dichas variables como entrada en un modelo predictivo.
- Proponer un módelo de regresión lineal múltiple para estimar la variable *price*.
<br/>

### Objetivos específicos : Pasos
1. Carga de datos y conversión de los mismos a dataframe.

2. Separación de los datos en dos grupos: training + control y testing.
    - El grupo training + control, con el cual se entrenará el modelo, contiene el 70% de los datos. 
    - El grupo test contiene el 30% de los datos, y se utilizará como simulación de datos reales una vez el modelo esté cerrado.
   
3. Realización de un análisis exploratorio inicial de cada una de las variables utilizando las observaciones del grupo de training.
    - Durante esta fase se decidirá qué variables se utilizarán, en principio, para construir el modelo predictivo.
    
4. Imputación de datos faltantes.

5. Transformaciones necesarias a cada una de las variables.

6. Aplicación de un modelo de regresión lineal múltiple con las variables seleccionadas para la predicción de la variable *price*.

7. Evaluación del modelo.

8. Repetición de los pasos de imputación y transformación para el grupo de test.

9. Aplicación del modelo sobre los datos de test.
...
Cuanta más experiencia tiene un vendedor, más confianza genera en los consumidores. Como consecuencia se genera más demanda, lo que tiene un efecto positivo en el precio. Las reseñas con fechas antiguas generan dudas en el consumidor, y no reflejan información actual. En consecuencia, la demanda cae, lo que afecta negativamente al precio.
...
```{r data, fig.height=5.5, fig.width=11, warning=FALSE}
scores <- data_train %>%
...
```{r host, warning=FALSE}
# Host exp days
host_since <- ggplot(data_train, aes(x=host_exp_days)) + geom_histogram(bins = 50, fill="darkslategray", col='white') + theme(legend.position = "none")
...
### 04.01 Análisis Multivariante

#### Variable price vs variables de interés

```{r priceVSroomtype, warning=FALSE, message=FALSE}
# Price vs room type
price_rtyp <- ggplot(data_train, aes(x=room_type, y=price)) + geom_boxplot() + scale_y_continuous(trans='log10')

# price vs neighbourhood_group_cleansed
ggplot(data_train, aes(x=neighbourhood_group_cleansed, y=price)) + geom_boxplot() + scale_y_continuous(trans='log10') + coord_flip()# CHECK: relevant

# price vs host_since
price_hs <- ggplot(data_train, aes(x=host_exp_days, y=price)) + geom_col() + scale_y_continuous(trans='log10') # CHECK: Irrelevant TODO: agregar por promedio

# price vs host_is_superhost
price_hisph <- ggplot(data_train, aes(x=host_is_superhost, y=price)) + geom_boxplot() + scale_y_continuous(trans='log10') #CHECK: bit of an edge
...
price_acc <- ggplot(pricevsAcco, aes(x=accommodates, y=precio)) + geom_col() # CHECK: Fairly obvious
...
price_bedr <- ggplot(pricevsBR, aes(x=bedrooms, y=precio)) + geom_col() # CHECK: accommodates are better
...
price_bds <- ggplot(pricevsBeds, aes(x=beds, y=precio)) + geom_col() # CHECK: Fairly obvious. Accommodates can be better.
...
price_nmin <- ggplot(pricevsminimum_nights, aes(x=minimum_nights, y=precio)) + geom_col() # No value
...
price_minavgn <- ggplot(pricevsminimum_nights_avg_ntm, aes(x=minimum_nights_avg_ntm, y=precio)) + geom_col() + scale_y_continuous(trans='log10') #¿?
...
price_maxavgn <- ggplot(pricevsmaximum_nights_avg_ntm, aes(x=maximum_nights_avg_ntm, y=precio)) + geom_col() # No value
...
price_ava360 <- ggplot(pricevsavailability_365, aes(x=availability_365, y=precio)) + geom_col() # CHECK: can seasonality be borrowed from here and scraping date?

# reviews_per_month vs price
price_rvpm <- ggplot(data_train, aes(x=reviews_per_month, y=price)) + geom_point()

# review_scores_rating vs price
price_rscr <- ggplot(data_train, aes(x=review_scores_rating, y=price)) + geom_point()
...
Preparación de variables para ploteado de heatmap. Nuevas variables de mean_ratings y mean_price con la media de cada una de estas variables por distrito, para su comparación con el precio. Además, añadimos una variable más que no se encuentra en la tabla de datos original, el número de habitantes por distrito en 2017.
...
## 05 Estudio e imputación de valores faltantes
...
## (KATIA, EL PAQUETE DMWR LO HAN QUITADO DE CRAN, PODEMOS USAR OTRO?)
...
Podemos apreciar que los datos imputados son muy cercanos a lo datos observados, pero el mejor resultado para las variables lo da el methodo *cart*. Usamos este methodo para imputación de valores faltantes (NA) en nuestras variables.
...
## 06 Ánálisis de outliers y valores atípicos
<br/>
...
## 07 Transformación de variables y eliminación de outliers

### 07.01 Transformación
...
## 07.03 Mapa con ajustes a la variable price
...
## 08 Modelo de Regresión Lineal: Ajuste, interpretación y diagnósis
<br/>

### 08.01 Correlación entre variables "a ojo"
...
## 08.02 Regresión lineal multiple
