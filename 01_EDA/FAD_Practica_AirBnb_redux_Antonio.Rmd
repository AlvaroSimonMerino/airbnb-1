...
Para la realización de esta práctica se ha seleccionado el dataset *listings.csv*  (Airbnb Madrid, 2021-09-10), un conjunto de datos obtenido de *insideairbnb.com*.  Puede ser descargado a través de este [enlace](http://data.insideairbnb.com/spain/comunidad-de-madrid/madrid/2021-09-10/data/listings.csv.gz).
...
2. Realización de un análisis exploratorio univariante de los datos.
3. Realización de un análisis exploratorio multivariante de los datos.
4. Control de calidad del dato.
...
El conjunto de datos está formado por un total de `r nrow(RawData)` datos y `r ncol(RawData)` variables:
...
```{r variablesOfInterest, echo=FALSE, warning=FALSE}
...
str(data)
```
Se comprueba que las variables $host_since$, $last_review$, $host_is_superhost$ y $price$ tienen asignado el tipo que les corresponde. Debido a ello se les asignarán los siguientes tipos:
<br/>

   - *Date* para $host\_since$, $last\_review$:
...
<br/>
   ...
   - *logical* para $host\_is\_superhost$:
```{r host_super, warning=FALSE}
...
#### Corrección de valores erróneos e imputación de valores faltantes.
