...
for (row in 1:nrow(data2)){
  ...
  for (element in 1:nrow(amenities)){
    if(str_detect(data2$amenities[row],amenities$name[element])){
      points = points + amenities$ratio[element]
    }
  }
  data2$am_score[row] <- points
}
...
data[grep("wanda", tolower(data3$description)) & data3$room_type=="Shared room" & data3$neighbourhood_group_cleansed=="San Blas - Canillejas",c("neighbourhood_group_cleansed","neighbourhood_cleansed")] <- "Wanda Metropolitano"

summary(lm(log(price) ~ log(accommodates)+log(bedrooms)+neighbourhood_cleansed + am_score, data = data3))

summary(lm(am_score ~ log(price), data = data3))
