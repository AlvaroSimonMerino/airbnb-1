...
En esta misma gráfica, podemos ver que se podría obtener un modelo con 16 predictores aproximadamente, cuyo error sería muy similar al del mejor modelo (que utiliza 39).
...
La información que obtenemos al aplicar regularización Lasso es parecida a la que obteníamos con las técnicas aplicadas anteriormente. Se obtiene un SE de 0.001511  con 16 variables, y 0.001375 con 39.
...
fit_m4 <- lm(log(price) ~ accommodates:district_price_group+dist_sol+ +reviews_per_month:district_price_group + host_listings_count, data = data_train_sel) ### 0.6253
...
fit_m5 <- lm(log(price) ~ accommodates:district_price_group+ dist_sol +reviews_per_month:district_price_group + host_listings_count+review_scores_rating, data = data_t_m) ### 0.6253
