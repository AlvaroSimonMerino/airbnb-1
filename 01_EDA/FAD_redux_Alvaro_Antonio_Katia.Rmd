...
Esta declaración nos permite concluir que es más lógico para construir una regresión lineal considerar solo las ubicaciones en habitaciones de hotel o apartamentos (casas).
En primer lugar, estos tipos de alojamiento son los más característicos de AirBNB, en segundo lugar, tienen una distribución similar del precio por persona por noche.
...
Y también vemos una dependencia diferente para diferentes tipos de alojamiento, donde los hoteles, habitaciones privadas y apartamentos tienden a reducir el precio con el aumento de la distancia, y para las habitaciones compartidas - al revés.
Al mismo tiempo, los hoteles tienden a reducir el precio más bruscamente que para habitaciones individuales o apartamentos.
...
data_train %>%
  ggplot(aes(x=(reorder(neighbourhood_group_cleansed, price, FUN=mean)), y=price, fill=neighbourhood_group_cleansed)) + geom_boxplot() + scale_y_continuous(trans='log10') + coord_flip() + theme(legend.position="none") + xlab("Distritos") + ylab("Precio")
...
Resulta que la palabra "wanda" está incluída en la mayoría.
"wanda" se refiere al estadio "Wanda Metropolitano", el estadio del Club Atlético de Madrid, que se encuentra en el límite del distrito.
...
Sobre la base del precio promedio por persona, combinamos los distritos de la ciudad en 5 grupos: - A - Wanda Metropolitano; - B - Chamartín, Salamanca, San Blas - Canillejas, Chamberí; - C - Hortaleza, Centro, Tetuán, Moncloa - Aravaca, Retiro, Fuencarral - El Pardo; - D - Arganzuela, Ciudad Lineal,Carabanchel,Villa de Vallecas,Barajas, Latina, Usera, Moratalaz, Vicálvaro; - E - Puente de Vallecas, Villaverde.
...
Aquí queremos señalar que si no limitamos el precio máximo por persona y noche a 143 euros, esta diferencia es varias veces mayor.
Esto se debe a que las ofertas más caras eran propias en el apogeo del turismo y, en consecuencia, en un momento sin restricciones COVID-19.
...
Ya que hemos definido previamente que la distribución del precio por persona por noche para habitaciones privadas y apartamentos individuales es bastante similar y cercana a la normalidad.
Esta declaración nos permite concluir que es más lógico para construir una regresión lineal considerar solo las ubicaciones en habitaciones de hotel o apartamentos (casas).
Por lo tanto, dejamos solo las observaciones para estos tipos de alojamento.
...
La mejor opcion es el modelo 8, con los siguientes coeficientes:
...
Podemos concluir que las 3 técnicas de selección automática de variables indican que el número óptimo de predictores es 8.
...
Atendiendo a nuestra valoración como expertos, vamos a probar a eliminar las observaciones dónde el precio por noche es inferior a 3 euros.

```{r select add, echo=FALSE}
data_train_sel <- subset(data_train_sel, price_per_person >= 3)
```

Vamos a comprobar como ha afectado este cambio, usando de nuevo uno de los métodos anteriores y nuestra nueva variable modificada.
...
Por tant concluímos que este modelo es el más adecuado para nuestros objetivos, por lo que procedemos a la evaluación del modelo con los datos de *test*.
...
Сomo resultado, obtuvimos el MAPE - 8,8%, que es que ligeramente peor que para el *train* (8,6%). Pero al mismo tiempo, el resultado es bastante bueno. 

Podemos concluir que el modelo seleccionado funciona casi por igual tanto en datos de *train* como en datos de *test*.

Resumen: Como resultado, vemos que el modelo resultante explica solo el 62,0% del precio por noche para un alojamiento (habitación individual o фpartamento), con MAPE a 8,6%. Cuando se eliminaron los valores atípicos para construir un modelo lineal, el impacto de la pandemia en los precios fue insignificante, como que durante el período sin restricciones los precios fueron significativamente más altos y alcanzaron su punto máximo. También relativamente bajo el valor de R² sugiere que tal vez todavía hay factores que afectan significativamente el precio que no están presentes en nuestro conjunto de datos.

En el ejemplo del cálculo de valores perdidos, se vio claramente que los métodos basados en *random forest* funcionan mejor en estos datos.
