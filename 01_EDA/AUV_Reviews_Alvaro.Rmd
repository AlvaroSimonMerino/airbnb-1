...
rs_rating_sh <- ggplot(data, aes(x = review_scores_rating, y=..count.., fill = host_is_superhost)) + 
...
corr <- data_train %>% select(price, accommodates, review_scores_rating, neighbourhood_group_cleansed, property_type, room_type, host_is_superhost)
...
#write.table(csv, file = "dataforheatmap.csv",
#            sep = "\t", row.names = F)
```
<br/>
...
ggplot(data_train, aes(x=longitude, y=latitude, color=price)) + geom_polygon(data = spdf_fortified, aes( x = long, y = lat, group = group), fill="white", color="black", alpha = 0.5) + theme_void() + coord_map() + geom_point() + ggtitle('Viviendas localizadas según su coordenada') + geom_polygon(data = spdf_fortified, aes( x = long, y = lat, group = group), color="black", alpha = 0)
