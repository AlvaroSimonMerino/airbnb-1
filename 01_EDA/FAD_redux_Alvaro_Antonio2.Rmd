---
title: "Modelo para predecir el precio de los AirBnb en Madrid"
...
Se comprueba que la densidad de observaciones en el distrito Centro de Madrid es sumamente desproporcionada en comparación con el resto. Solo en este distrito se acumulan un `r round(count(data_train[data_train$neighbourhood_group_cleansed=="Centro",])*100/nrow(data_train),2)`% de los espacios alquilables de Madrid (casi la mitad).
...
Resulta interesante comprobar el descenso abrupto del número de reviews que se produce el 15 de Marzo de 2020, primer día de Estado de Alarma en España. Según los datos, el `r rBC_pct`% de los espacios no han recibido una review hasta el día en que se recolectaron los datos (10 de Septiembre). Podría resultar interesante comparar los precios de los espacios en función de si han sido evaluados antes o después de esta fecha.
...
```{r grafico_na, echo=FALSE, fig.height=5, fig.width=7.5, warning=FALSE}
...
```{r fig.height=7.5, fig.width=8, warning=FALSE, fig.align="center",eval=FALSE}
...
ggplot(data_train, aes(x=reorder(neighbourhood_group_cleansed, mean(neighbourhood_group_cleansed)), y=price, fill=neighbourhood_group_cleansed)) + geom_boxplot() + scale_y_continuous(trans='log10') + coord_flip() + theme(legend.position="none")
